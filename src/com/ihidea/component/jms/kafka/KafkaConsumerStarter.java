...
public class KafkaConsumerStarter {
    ...
    private synchronized static void initConsumerMethod() throws Exception{
        ...
        Map<String, Object> kafkaConsumerClz = SpringContextLoader.getBeansWithAnnotation(com.ihidea.component.jms.kafka.KafkaConsumer.class);

        if (kafkaConsumerClz != null) {
            for (Object clzObj : kafkaConsumerClz.values()) {

                List<Method> methodList = ClassUtilsEx.getClassMethodByAnnotation(clzObj.getClass(), KafkaConsumerMethod.class);

                for (Method method : methodList) {

                    KafkaConsumerMethod methodAnno = method.getAnnotation(KafkaConsumerMethod.class);

                    String topicName = methodAnno.topicName();
     ...
     * @param subscribedTopics      订阅的topic集合
     ...
     */
    public void init(String brokerAddress, String consumerGroupName, List<String> subscribedTopics, int sessionTimeOutMs, int maxPollRecords, int consumerThreadNum) throws Exception {
